...
class CensoredNormalDistribution(Distribution):
	...
	def log_density(self, data):
		...
		log_density = np.where(np.logical_or(data < self.a, data > self.b) , -np.inf, log_density)
		log_density = np.where(data == self.a, 500*self.norm_dist.cdf(self.a) , log_density)
		log_density = np.where(data == self.b, 500*(1-self.norm_dist.cdf(self.b)), log_density)
	...
	def estimate_parameters(self, data, weights):
		...
		wsum = np.sum(weights)
		moments = Moments(self.mu, self.sigma, self.a, self.b)
		...
		new_data1 = np.where(data <= self.a, left_trunc_moments.first_moment, data)[:, np.newaxis]
		new_data = np.where(new_data1 >= self.b, right_trunc_moments.first_moment, new_data1)
