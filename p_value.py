...
class Data_Rep:
	...
	def _mixture_cdf(self, data, dist_list):
		weights, distributions, log_l = mixem.em(data, dist_list, progress_callback=None)
	...
	def get_scipy_dists(distributions):
		...
		for dist in distributions:
			...
			if dist_type == mixem.distribution.NormalDistribution:
				...
				loc = dist.mu
				scale = dist.sigma
				ss_dist = truncnorm(a=(a-loc)*1.0/scale, b=(b-loc)*1.0/scale, loc=loc, scale=scale)
...
if __name__ == "__main__":
	...
	predata1 = np.random.normal(loc=0, scale=1, size=1000000)
	data1 = np.array(list(filter(lambda x: x > -1 and x < 1, predata1)))

	predata2 = np.random.normal(loc=3, scale=1, size=1000000)
	...
	plt.plot(x, scipy_dist_1.pdf(x), label='-1 to 1 pdf')
	plt.plot(x, scipy_dist_2.pdf(x), label='2 to 4 pdf')
	plt.plot(x, post_scipy_dist_1.pdf(x), label= 'post %s to %s pdf' % (
		post_scipy_dist_1.a, post_scipy_dist_1.b
	))
	plt.plot(x, post_scipy_dist_2.pdf(x), label= 'post %s to %s pdf' % (
		post_scipy_dist_2.a, post_scipy_dist_2.b
	))

	plt.hist(data, bins=200, normed=True, label='Real Data Histogram.')
	plt.legend()
	plt.show()
